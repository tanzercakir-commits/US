"""Generate or check deterministic runtime-contract wrapper artifacts."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate exact enforcement-ladder runtime wrappers."
    )
    ...
    try:
        ...
        print(f"runtime-assertions error: {error}", file=sys.stderr)
        return 2

    outputs = (
        (args.wrapper, bundle.wrapper),
    ...
    if args.check:
        for path, rendered in outputs:
            try:
                ...
                print(f"runtime-assertions check error: {error}", file=sys.stderr)
                return 2
            if rendered != expected:
                print(
                    f"runtime-assertions output differs from {path.as_posix()}",
                    file=sys.stderr,
                )
                return 1
        print("runtime-assertion artifacts match")
        return 0
    ...
    try:
        ...
        print(f"runtime-assertions output error: {error}", file=sys.stderr)
        return 2
    return 0
